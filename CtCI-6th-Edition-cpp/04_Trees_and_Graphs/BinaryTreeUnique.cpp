...
p_node_t create(const std::string& tree) {
  ...
  p_node_t root;
  BinaryTreeUniqueNode* p = nullptr;
  std::vector<BinaryTreeUniqueNode*> stack;
  ...
  while (begin < tree.size()) {
    ...
    if (token == "null") {
      ...
      if (p == nullptr) {
        if (stack.empty()) {
          root = std::make_unique<BinaryTreeUniqueNode>(v);
          stack.push_back(root.get());
        } else {
          stack.back()->left = std::make_unique<BinaryTreeUniqueNode>(v);
          stack.push_back(stack.back()->left.get());
        }
      } else {
        p->right = std::make_unique<BinaryTreeUniqueNode>(v);
